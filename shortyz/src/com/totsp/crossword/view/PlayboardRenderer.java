        ...
        try {
            ...
            for (int col = 0; col < boxes.length; col++) {
                for (int row = 0; row < boxes[col].length; row++) {
                    ...
                    Paint boxColor = highlight.across == col && highlight.down == row ? this.currentLetterBox : this.blackLine;
                    ...
                    if (boxes[col][row] == null) {
                        ...
                        if (boxes[col][row].across | boxes[col][row].down) {
                            canvas.drawText(Integer.toString(
                                    boxes[col][row].clueNumber), startX + 2,
                                startY + numberTextSize + 2, this.numberText);
                        }

                        canvas.drawText(Character.toString(
                                boxes[col][row].response),
                            startX + (boxSize / 2),
